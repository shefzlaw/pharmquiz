  ...
  try {
    ...
    if (username && sessionToken) {
      ...
      if (response.ok) {
        ...
        if (data.subscription && data.subscription.end > now) {
          isSubscribed = true;
          subscriptionEnd = data.subscription.end;
          showStartScreen();
        } else {
          isSubscribed = false;
...
async function registerUser() {
  ...
  try {
    ...
    const data = await response.json();
    if (response.ok) {
      showMessage("register-error", data.message, true);
      document.getElementById("register-username").value = "";
      document.getElementById("register-password").value = "";
      showLoginScreen();
    } else {
      showMessage("register-error", data.error);
    }
  } catch (error) {
...
async function submitAccessCode() {
  ...
  try {
    ...
    if (response.ok) {
      isSubscribed = true;
      subscriptionEnd = data.subscription.end;
      showMessage("subscription-error", data.message, true);
      showStartScreen();
    } else {
      showMessage("subscription-error", data.error);
    }
  } catch (error) {
...
async function logoutUser() {
  ...
  try {
    ...
    showMessage("login-error", "Logout failed. Please try again.");
  }
}
  ...
  if (currentUser) {
    ...
    try {
      const response = await fetch('/api/check-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: currentUser.username, sessionToken }),
      });
      if (!response.ok) {
        logoutUser();
...
function startQuiz() {
  ...
  try {
    ...
    const availableQuestions = quizData[currentCourse]?.length || 0;
    const maxQuestions = isSubscribed ? Math.min(questionCount, availableQuestions) : Math.min(15, availableQuestions);
    if (availableQuestions === 0) {
      showMessage("start-error", `No questions available for ${currentCourse}.`);
      return;
    }
    if (isSubscribed && ![25, 50, 100].includes(questionCount)) {
      showMessage("start-error", "Please select 25, 50, or 100 questions.");
      return;
    } else if (!isSubscribed && questionCount !== 15 && questionCount !== availableQuestions) {
      showMessage("start-error", "Free users can only select 15 questions (or available questions if fewer).");
      return;
    }
    if (questionCount > availableQuestions) {
      showMessage("start-error", `Only ${availableQuestions} questions available for ${currentCourse}. Selecting ${availableQuestions} questions.`);
      questionCount = availableQuestions;
    }
    currentQuestion = 0;
    score = 0;
    answered = [];
    selectedQuestions = isSubscribed ? shuffleArray([...quizData[currentCourse]]).slice(0, questionCount) : quizData[currentCourse].slice(0, maxQuestions);
